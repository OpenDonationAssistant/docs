...
Optionally, if your widget needs runtime data (WebSocket subscriptions, API calls):

| 7 | `src/pages/MyWidget/MyWidgetStore.ts` | Real store — connects to backend | | 8 | `src/pages/MyWidget/MyWidgetDemoStore.ts` | Demo store — used in settings preview |
